...
export class MessageService {
  ...
  private getChatsFromMessages(result: MessagesResponse) {
    ...
    if (result.totalCount > 0) {
      ...
      if (this.authService.getRole() == "ADMIN")
        loggedUserId = 0;
      let firstIndex = 0, currentOtherUserId,
        lastOtherUserId = (result.results.at(0)?.receiverId || 0) + (result.results.at(0)?.senderId || 0) - loggedUserId;
  ...
  private createChat(result: MessagesResponse, firstIndex: number, lastIndex: number): Chat {
    ...
    for (let i = firstIndex; i < lastIndex; i++) {
      ...
      chat.messages.push({
        timestamp: toDate(message?.timeOfSending) || new Date(),
  ...
  sendMessage(request: MessageRequest, receiverId: number) {
    this.sendMessageToBack(request, receiverId).subscribe({
      next: (result) => {
        ...
        const chat: Chat = chats.find(object => {
          return object.rideId === result.rideId && (object.receiverId === result.receiverId || object.receiverId === result.senderId)
        }) || {image: '', name: '', messages: [], rideId: -1, receiverId: -1};
        chat.messages.push({
          timestamp: toDate(result.timeOfSending),
  ...
  public sendMultipleMessageToBack(message: MessageRequest, userIds: number[]): Observable<string> {
    return this.http.post(
      environment.apiHost + "user/send-messages",
      {"message": message, "userIds": userIds},
      {
        responseType: "text"
      }
    );
...
function toDate(str: object | Date | undefined): Date {
  return <Date>str;
}
